...
knitr::opts_chunk$set(echo = TRUE, error = TRUE, python.reticulate=F)
...
```{bash eval=FALSE, echo=FALSE, include=FALSE}
...
hm, worried about the clobbering of distance matricies as written


```{r echo=FALSE, include=FALSE, warning=FALSE}
#ordinarily this would come from the config file: 
#	data_sets.df %>% filter(subroups == "all") %>% order_by(experimental/control/popSim)


#all_droSim1_w100000_s100000 <- read_delim("dev/distances/10A/all.droSim1_w100000_s100000.tbl", "\t", escape_double = FALSE, trim_ws = TRUE)



samp_list <- c("SECH1", "SECH2", "SECH3", "SECH4", "MD06m11d04y2010", "MD199m12d10y2010", "MD73m11d04y2010", "NS79m03d07y2011", "SRR5860570", "10A", "10B", "17A", "17B", "SRR303333")

df_list <- vector("list", length(samp_list))
i <- 1
for (samp in samp_list) {
	df.tmp <- read_delim(paste("dev/distances/", samp, "/all.droSim1_w100000_s100000.tbl", sep=""), "\t", escape_double = FALSE, trim_ws = TRUE)
	df_list[[i]] <- df.tmp
	i <- i+1
}
...
melted_matt <- melt(matt_list[[122]])
ggplot(data = melted_matt, aes(x=Var1, y=Var2, fill=1-value)) + geom_tile()+ scale_fill_gradient(limits=c(0.4,1))
```
...
for (matt in seq(1,length(matt_list) )) {

	print(matt)
	melted_matt <- melt(matt_list[[matt]])
	matt_plot<- ggplot(data = melted_matt, aes(x=Var1, y=Var2, fill=1-value)) + geom_tile()+ scale_fill_gradient(limits=c(0.2,1))
	
	
	matt_plot
	ggsave(file=paste("dev/heatmaps/",matt,".png", sep=""))
}

```


big peaks in windows #122-123 and 179-180

```{bash eval=FALSE, echo=FALSE}

png_list=$(seq 1 221 | awk '{print "dev/heatmaps/"$1".png"}' | tr "\n" " ")
convert -delay 35 -loop 1 $png_list dev/heatmaps/animated.gif
```
...
https://htmlpreview.github.io/?https://github.com/talgalili/dendextend/blob/master/inst/ignored/Introduction%20to%20dendextend.html
...
```{r echo=FALSE}
matt<-122
matty <- matt_list[[matt]]
denny <- hclust(matty %>%  as.dist()) %>% as.dendrogram() 
...
denny %>% set("labels_col",data_sets.df.tmp$label %>% as.character()) %>%  hang.dendrogram(hang_height = .25) %>% set("leaves_col", data_sets.df.tmp$label %>% as.character()) %>%  set("leaves_pch", 19) %>% set("leaves_cex", 2)  %>% as.ggdend() %>% ggplot() + ylim(c(-2,2))
```
...
for (matt in seq(1,length(matt_list) )) {
	...
	denny_plot <- denny %>% set("labels_col",data_sets.df.tmp$label %>% as.character()) %>%  hang.dendrogram(hang_height = .25) %>% set("leaves_col", data_sets.df.tmp$label %>% as.character()) %>%  set("leaves_pch", 19) %>% set("leaves_cex", 2)  %>% as.ggdend() %>% ggplot() + ylim(c(-2,2))
	
	denny_plot
	ggsave(file=paste("dev/phylotree/",matt,".png", sep=""))
}
...
Thank you for your service, Antonio Miguel de Jesus Dominigues, re: ggbio@ggplot call
https://support.bioconductor.org/p/68650/

```{r echo=FALSE, eval=FALSE}
chr2L.freqCompare.windowed.bg$winnum <- seq(1,221)
...
for (matt in seq(1,length(matt_list) )) {
	...
	melted_matt <- melt(matt_list[[matt]])
	matt_plot<- ggplot(data = melted_matt, aes(x=Var1, y=Var2, fill=1-value)) + geom_tile()+ scale_fill_gradient(limits=c(0.2,1))+ theme_clear()+ theme(legend.position="none") + labs(x="", y="")
	...
	chr2L.freqCompare.windowed.simIntrog.tk.tmp <- autoplot(chr2L.freqCompare.windowed.bg, aes(y=avg_sim_introg), geom='line') + geom_point(aes(y=avg_sim_introg, alpha=as.factor(winnum==matt)), color="red") + scale_alpha_discrete(range=c(0,1))+ theme_clear()+ theme(legend.position="none")
	
	gee.tmp<-grid.arrange(chr2L.freqCompare.windowed.simIntrog.tk.tmp@ggplot, matt_plot, denny_plot, nrow = 2, layout_matrix = cbind(c(1,2), c(1,3)))
	
 ggsave(plot=gee.tmp, file=paste("dev/chr2Lflyby/",matt,".png", sep=""))
...
working on reflowing the frequency shift code into snakemake. Some manipulations done via bedtools & vcftools, others will be in a standalone R script (include in the summary PDF?)
...
```{bash eval=FALSE}
Rscript scripts/freqShifter.R potato tomato
```


## 9 Jan 2019

ok now gonna plot the frequency shifts and such
...
freqCompare.windowed.snpCount.tk <- autoplot(windowedFreqShift.bg.chr2L, aes(y=num_snp, color=treatment), geom='line') + theme_clear()
freqCompare.windowed.simIntrog.tk <- autoplot(windowedFreqShift.bg.chr2L, aes(y=avg_sim_introg, color=treatment), geom='line') + theme_clear()
freqCompare.windowed.secDeplet.tk <- autoplot(windowedFreqShift.bg.chr2L, aes(y=avg_sec_depl, color=treatment), geom='line') + theme_clear()

tks <- tracks( number_SNPs = freqCompare.windowed.snpCount.tk, simulans_similarity = freqCompare.windowed.simIntrog.tk, sechellia_dissimilarity = freqCompare.windowed.secDeplet.tk )
```
...
autoplot(windowedFreqShift.bg.autosomal, aes(y=avg_sim_introg, color=treatment), geom='line') + facet_wrap(~seqnames) + theme_clear()

autoplot(windowedFreqShift.bg.autosomal, aes(y=avg_sec_depl, color=treatment), geom='line') + facet_wrap(~seqnames) + theme_clear()
...
working on integrating the freq shifts / clusters into the main report.

## 14 January 2019

Titling a table grob: https://stackoverflow.com/questions/31640916/how-can-i-add-a-title-to-a-tablegrob-plot


## 22 January 2019

Gotta speed up the variant calling
https://groups.google.com/forum/#!topic/freebayes/AGBvsqZIxE4

## 23 January 2019

had to edit the freebayes-parallel script to take explicit file names for the vcflib commands, and also download the GNU Parallel package and add its location to my $PATH:
/nas/longleaf/home/csoeder/modules/parallel/bin/

sbatch -n 8 -t 1:00:00 --mem=16G --wrap=" ~/modules/freebayes/scripts/freebayes-parallel <(~/modules/freebayes/scripts/fasta_generate_regions.py /proj/cdjones_lab/Genomics_Data_Commons/genomes/drosophila_simulans/droSim1.fa 100000) 8 --standard-filters -f /proj/cdjones_lab/Genomics_Data_Commons/genomes/drosophila_simulans/droSim1.fa mapped_reads/MD06m11d04y2010.vs_droSim1.bwaUniq.sort.bam "


## 24 January 2019

SLURM won't cooperate with that subprocess :( 

~/modules/freebayes/scripts/fasta_generate_regions.py /proj/cdjones_lab/Genomics_Data_Commons/genomes/drosophila_simulans/droSim1.fa 100000 > test.test 

sbatch -n 1  -t 1:00:00 --mem=16G --wrap="~/modules/freebayes/scripts/freebayes-parallel test.test 1 --standard-filters -f /proj/cdjones_lab/Genomics_Data_Commons/genomes/drosophila_simulans/droSim1.fa mapped_reads/MD06m11d04y2010.vs_droSim1.bwaUniq.sort.bam > potato.vcf "

ugh gotta rewrite the fasta_generate_regions.py to be python3 compatible though???

nah ditch it entirely and reformat the windowed file. 


Oh great I gotta rewrite the vcflib scripts for python3 too

## 25 January 2019

ok yeah the parallel processing is a game changer!!! too bad I keep forgetting to change the setwd() command before pushing to git !!!!!!!!!!!!!!!!!!

From: Soeder, Charles
Sent: Monday, January 21, 2019 12:47 AM
To: Earley, Eric; Jones, Corbin D
Subject: Re: another test of selection

1) how to exclude incomplete lineage sorting?
2) timing of introgression from sim: before or after domestication?
a) in the first case, the variation would appear in the natural population as well as the laboratory populations sampled from it. So far this has largely been a contrast between lab and wild flies in the areas that I pulled out, but that might also have to do with the order of samples I've added. 
b) in the second case, where the introgression occurs after the flies are brought into the lab (maybe some sims get into the wrong vial or something). Then, no wild sechellias will be sim-like but some lab sechellias will. 
3) if the introgression is common in the lab but not in the wild, it might be an adaptation to lab conditions. but, the population genetic statistics we've been looking at have been drawn from the wild populations (so we might not see marks of selection there!)


May be useful: ggtree
https://bioconductor.org/packages/release/bioc/vignettes/ggtree/inst/doc/treeVisualization.html
