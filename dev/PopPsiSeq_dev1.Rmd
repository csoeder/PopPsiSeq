...
```{r echo=FALSE, include=FALSE}
depth.process %>% group_by(reference)%>% summarise(step="pre-filtration depth",minimum = min(bwa), average=mean(bwa), median = median(bwa), maximum = max(bwa)) 
```
...
breadth.process <- all_alignments %>%  filter(measure=='total_breadth' ) %>% spread(aligner, value) %>%  mutate(breadth_retention = 100*bwaUniq/bwa)
```
