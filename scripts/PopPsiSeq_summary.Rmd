...
```{r echo=FALSE}
before_After.counts.aug %>% gather(mapping_retention:filter_retention, key="measure", value="value") %>%  group_by(measure) %>% summarise(minimum = min(100*value), average=mean(100*value), median = median(100*value), maximum = max(100*value)) %>% kable(caption="Percentage of Reads Retained at Each Step",digits=1)
